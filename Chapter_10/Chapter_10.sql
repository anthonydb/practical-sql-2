...
-- 코드 10-1: Importing the FSIS Meat, Poultry, and Egg Inspection Directory
...
-- 코드 10-2: Finding multiple companies at the same address
SELECT company,
       street,
       city,
       st,
...
-- 코드 10-3: Grouping and counting states
SELECT st, 
       count(*) AS st_count
FROM meat_poultry_egg_establishments
GROUP BY st
...
-- 코드 10-4: Using IS NULL to find missing values in the st column
...
-- 코드 10-5: Using GROUP BY and count() to find inconsistent company names
...
-- 코드 10-6: Using length() and count() to test the zip column
...
-- 코드 10-7: Filtering with length() to find short zip values
...
-- 코드 10-9: Creating and filling the st_copy column with ALTER TABLE and UPDATE
...
-- 코드 10-10: Checking values in the st and st_copy columns
...
-- 코드 10-11: Updating the st column for three establishments
...
-- 코드 10-12: Restoring original st column values
...
-- 코드 10-13: Creating and filling the company_standard column
...
-- 코드 10-14: Using an UPDATE statement to modify column values that match a string
...
-- 코드 10-15: Creating and filling the zip_copy column
...
-- 코드 10-16: Modify codes in the zip column missing two leading zeros

UPDATE meat_poultry_egg_establishments
SET zip = '00' || zip
WHERE st IN('PR','VI') AND length(zip) = 3;

-- 코드 10-17: Modify codes in the zip column missing one leading zero

UPDATE meat_poultry_egg_establishments
SET zip = '0' || zip
WHERE st IN('CT','MA','ME','NH','NJ','RI','VT') AND length(zip) = 4;

-- 코드 10-18: Creating and filling a state_regions table
...
-- 코드 10-19: Adding and updating an inspection_deadline column
...
-- 코드 10-20: Viewing updated inspection_deadline values
...
-- 코드 10-21: Deleting rows matching an expression

DELETE FROM meat_poultry_egg_establishments
WHERE st IN('AS','GU','MP','PR','VI');

-- 코드 10-22: Removing a column from a table using DROP

ALTER TABLE meat_poultry_egg_establishments DROP COLUMN zip_copy;

-- 코드 10-23: Removing a table from a database using DROP

DROP TABLE meat_poultry_egg_establishments_backup;

-- 코드 10-24: Demonstrating a transaction block
...
-- 코드 10-25: Backing up a table while adding and filling a new column
...
-- 코드 10-26: Swapping table names using ALTER TABLE
